...
const DeltaVersioning = ({ selectedTable }: DeltaVersioningProps) => {
  ...
  const [commits, setCommits] = useState<{ fileName: string; path: string }[]>(
    []
  );
  ...
  const fetchCommits = async (selectedTable: string) => {
    ...
    try {
      ...
      setCommits(commits);
      setSelectedFile1({
        fileName: latestCommit.fileName,
        path: latestCommit.path,
      });
      setSelectedFile2({
        fileName: oldestCommit.fileName,
        path: oldestCommit.path,
      });
    } catch (error) {
  ...
  const fetchSchema = async (
    ...
    setLoading(true);
    try {
      console.log(`Schema ::: ${basePath}/${filePath}`);
      const schemaResponse = await apiClient.post("/delta/schema", {
        fileDirectory: `${basePath}${filePath}`,
      });
  ...
  useEffect(() => {
    if (selectedFile1.path) fetchSchema(selectedFile1.path, setSchema1);
  }, [selectedFile1]);

  useEffect(() => {
    if (selectedFile2.path) fetchSchema(selectedFile2.path, setSchema2);
  }, [selectedFile2]);
    ...
    <div className="p-6 bg-gray-100 min-h-screen">
      ...
      <div className="flex gap-4 mb-6">
        <div>
          ...
          <Select
            value={selectedFile1.path}
            onValueChange={(path) => {
              const commit = commits.find((c) => c.path === path);
              if (commit) setSelectedFile1(commit);
            }}
          >
        ...
        <div>
          ...
          <Select
            value={selectedFile2.path}
            onValueChange={(path) => {
              const commit = commits.find((c) => c.path === path);
              if (commit) setSelectedFile2(commit);
            }}
          >
            <SelectTrigger className="w-60 bg-white border border-gray-300 p-2 rounded">
