...
export const checkpointSchema = async (req: Request, res: Response) => {
    try {
        let { accessKey, secretKey, region, endpoint, urlStyle = "path", useSSL = "false", filePath } = req.body as CheckpointRequest;
...
`
        ...
        const result = await connection.run(`SELECT * FROM read_parquet('${filePath}');`);
